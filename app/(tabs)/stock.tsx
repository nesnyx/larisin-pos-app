    ...
    return (
      ...
      >
        <View className="flex-1 ml-4 justify-between h-20 py-1">
          <Text className="text-[10px] font-bold text-lime-600 uppercase tracking-widest mb-1">
            {item.category}
          </Text>
          <Text
            className="text-base font-bold text-gray-800 leading-5"
            numberOfLines={1}
          >
            {item.name}
          </Text>
        ...
        <View className="items-end justify-between h-20 py-1">
          ...
          <View
            className={`flex-row items-center px-3 py-1.5 rounded-xl ${isEmpty ? "bg-red-50" : isLowStock ? "bg-amber-50" : "bg-gray-50"}`}
          >
            {isLowStock || isEmpty ? (
              <AlertCircle size={12} />
            ) : (
            ...
            <Text
              className={`text-[11px] ml-1 font-bold ${isEmpty ? "text-red-600" : isLowStock ? "text-amber-600" : "text-gray-600"}`}
            >
              {item.stock}
            </Text>
          </View>
