...
const TransactionPage = () => {
    ...
    return (
        <View className="flex-1 bg-white" style={{ paddingTop: insets.top }}>
            {/* --- LAYAR UTAMA: LIST PRODUK --- */}
            <View className="px-6 py-4 flex-row justify-between items-center">
                <Text className="text-3xl font-black text-gray-900">Kasir</Text>
                <TouchableOpacity onPress={() => setIsCartVisible(true)} className="bg-gray-100 p-3 rounded-2xl relative">
                    <ShoppingBag size={24} color="#111827" />
                    {totalItem > 0 && <View className="absolute -top-1 -right-1 bg-lime-500 w-5 h-5 rounded-full items-center justify-center border-2 border-white"><Text className="text-[10px] text-white font-bold">{totalItem}</Text></View>}
                </TouchableOpacity>
            </View>
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <TouchableOpacity onPress={() => addToCart(item)} className="flex-1 m-2 bg-white border border-gray-100 p-4 rounded-[32px] shadow-sm">
            ...
            {totalItem > 0 && (
                <View className="absolute bottom-10 left-6 right-6 shadow-2xl shadow-lime-500/50">
                    <TouchableOpacity onPress={() => setIsCartVisible(true)} className="bg-lime-400 flex-row items-center justify-between p-5 rounded-[28px]">
                        <Text className="text-white font-bold text-base">{totalItem} Item | Rp {totalPrice.toLocaleString()}</Text>
                    ...
                    <View className="bg-white rounded-t-[40px] h-[80%]">
                        ...
                        <View className="px-8 py-6 border-t border-gray-50" style={{ paddingBottom: insets.bottom + 20 }}>
                            {cart.length === 0 ? <TouchableOpacity onPress={() => setIsCartVisible(false)} className="bg-slate-400 h-16 rounded-[24px] items-center justify-center">
                                <Text className="text-white font-bold text-md">Tidak ada Produk</Text>
                            </TouchableOpacity> : <TouchableOpacity onPress={() => setIsCheckoutVisible(true)} className="bg-gray-900 h-16 rounded-[24px] items-center justify-center">
                                <Text className="text-white font-bold text-lg">Lanjut Pembayaran</Text>
                            </TouchableOpacity>}
                        </View>
                    </View>
                </View>
            ...
            <Modal visible={isCheckoutVisible} animationType="fade" transparent={true}>
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
                    <View className="flex-1 bg-black/60 justify-center px-6">
                        <View className="bg-white rounded-[40px] p-8">
                            <View className="items-center mb-6">
                                <Text className="text-xl font-black text-gray-900">Detail Pembayaran</Text>
                                <View className="bg-gray-100 px-4 py-2 rounded-full mt-2">
                                    <Text className="text-gray-500 font-bold text-xs uppercase tracking-widest">Total Tagihan</Text>
                                </View>
                                <Text className="text-3xl font-black text-lime-600 mt-1">Rp {totalPrice.toLocaleString()}</Text>
                            </View>
                            ...
                            <View className="gap-y-4">
                                ...
                                <View className="bg-gray-50 p-5 rounded-3xl border border-dashed border-gray-200">
                                    <Text className="text-gray-400 font-bold text-center text-[10px] uppercase">Kembalian</Text>
                                    <Text className={`text-center text-2xl font-black mt-1 ${changeAmount < 0 ? 'text-red-400' : 'text-gray-900'}`}>
                                        Rp {changeAmount < 0 ? '0' : changeAmount.toLocaleString()}
                                    </Text>
                                    {changeAmount < 0 && <Text className="text-red-400 text-[10px] text-center font-bold mt-1 italic">* Uang belum cukup</Text>}
                                </View>
                            </View>
