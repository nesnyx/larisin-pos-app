...
const AuthPage = () => {
  ...
  const handleAuth = async () => {
    ...
    // Validasi Sederhana
    if (!email || !password)
      return notify("Waduh!", "Email & Password jangan kosong ya!");
    if (!isLogin && (!name || !confirmPassword))
      return notify("Data Kurang", "Semua kolom harus diisi!");
    if (!isLogin && password !== confirmPassword)
      return notify("Gak Cocok!", "Password konfirmasi beda nih.");
    ...
    try {
      if (isLogin) {
        ...
        notify(
          "Berhasil!",
          "Akun dibuat. Silakan login untuk verifikasi.",
          "success",
        );
        setIsLogin(true);
      ...
      notify(
        isLogin ? "Login Gagal" : "Gagal Daftar",
...
const InputField = ({ label, icon, rightIcon, ...props }: any) => (
  <View>
    <Text className="text-gray-400 font-bold text-[10px] uppercase ml-1 mb-2">
      {label}
    </Text>
